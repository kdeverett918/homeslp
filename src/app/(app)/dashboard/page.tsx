...
export default async function DashboardPage() {
  ...
  if (!profile?.onboarding_completed) redirect("/choose");
    ...
    <div className="space-y-8 max-w-4xl">
      ...
      {isTrialing && (
        <div className="rounded-xl border border-primary/20 bg-gradient-to-r from-rose-50 to-sage-50 p-4 flex items-center justify-between gap-4 flex-wrap">
          ...
          <Link
            href="/settings"
            className="inline-flex items-center gap-1.5 rounded-lg bg-primary px-4 py-2 text-sm font-medium text-primary-foreground shadow-warm-sm hover:bg-primary/90"
          >
            Upgrade Now
          </Link>
        </div>
      ...
      {!isPaid && (
        <div className="rounded-xl border-2 border-dashed border-primary/30 bg-primary/5 p-6 text-center space-y-3">
          <h2 className="font-heading text-lg font-semibold">
            Start your free trial to unlock content
          </h2>
          <p className="text-sm text-muted-foreground">
            Get 7 days free access to Week 1 Blueprint and the community.
          </p>
          <Link
            href="/pricing"
            className="inline-flex items-center gap-2 rounded-lg bg-primary px-6 py-2.5 text-sm font-medium text-primary-foreground shadow-warm-sm hover:bg-primary/90"
          >
            Start Free Trial
      ...
      <section>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          {blueprints.map((bp) => {
            const isAccessible =
              bp.accessTier === "trial" ? isPaid : bp.accessTier === "paid" ? subscription?.status === "active" : true;

            return (
              <Link
                key={bp.id}
                href={isAccessible ? `/blueprints/${bp.slug}` : "#"}
                className={`group relative rounded-xl border bg-card p-5 transition-all ${
                  isAccessible
                    ? "hover:shadow-warm-md hover:border-primary/30"
                ...
                <div className="flex items-start gap-3">
                  <div
                    className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${
                      isChild ? "bg-rose-100" : "bg-sage-100"
                    }`}
                  >
      ...
      <section>
        ...
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-4">
          {[
            ...
            {
              href: isChild ? "/blueprints" : "/recipes",
              icon: BookOpen,
              label: isChild ? "All Blueprints" : "Recipes",
            },
            {
              href: "/settings",
              icon: Sparkles,
