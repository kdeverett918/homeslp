...
export default function CheckPage() {
  ...
  const results = useMemo(() => {
    ...
    const redFlags = filteredQuestions.filter((q) => {
      // scr-26 is inverted: "Yes" (child lost words) is the red flag
      if (q.id === "scr-26") return answers[q.id] === true;
      // All other red-flag questions: "No" answer triggers the flag
    ...
    // Override: if any red flags exist and score says "low", bump to "some"
    const redFlagOverride = redFlags.length > 0 && level === "low";
    if (redFlagOverride) level = "some";

    return { percent, redFlags, breakdown, level, earnedWeight, totalWeight, redFlagOverride };
            ...
            <>
              <FadeIn delay={0.12}>
                <div className="rounded-xl border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">
                  ...
                  <p className="mt-1 text-xs">This screener is an educational tool, not a diagnostic instrument. Results do not replace professional evaluation by a licensed speech-language pathologist. If you have any concerns about your child&apos;s development, seek a professional evaluation regardless of these results.</p>
                </div>
              </FadeIn>
            ...
            <div className="space-y-4">
              ...
              <FadeIn>
                <div className={`rounded-xl border p-6 space-y-3 ${
                  ...
                  {results.redFlagOverride && (
                    <p className="text-xs text-amber-800 mt-2">
                      Your overall score is in the typical range, but some individual items may warrant professional discussion.
                    </p>
                  )}
                </div>
              </FadeIn>
                ...
                <div className="rounded-xl bg-gradient-to-br from-rose-50 to-sage-50 border border-rose-200/30 p-5">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm leading-relaxed">
                      This screener is not a diagnosis. It helps identify areas that may benefit from professional evaluation. While some children develop language on a different timeline, research shows early evaluation leads to better outcomes. Getting an evaluation is never the wrong choice.
                    </p>
                  </div>
                </div>
              </FadeIn>
              ...
              <div className="hidden print:block mt-6 border-t pt-4">
                <p className="text-xs italic">
                  This screening was generated by HomeSLP (homeslp.com), an educational platform.
                  It is NOT a diagnostic instrument and does not replace professional evaluation.
                  Always consult a licensed Speech-Language Pathologist for clinical assessment.
